...
%     fy = fy + sum(gp.P .* lls); % power posterior
    fy = fy + sum(lls);
end
f = ff + fy + fth;
...
for i = 1:gp.Nt
%     err = gp.P .* (gp.Y{i}-Xmu{i}); % power posterior
    err = (gp.Y{i}-Xmu{i});
...
dFy = zeros(size(dFf));
for i = 1:gp.Nt
%     dFy_i = gp.P'.* grad_sens(gp,gp.Y{i},Xmu{i},S{i}); % power posterior
